...
as
if @Action = 0 -- Nếu Action = 0, thực hiện thêm dữ liệu
	begin
		insert into Category([Name],[Type]) values(@Name,@Type)
		set	 @ID = @@IDENTITY -- Thiết lập ID tự tăng
	end
else if @Action = 1 -- Nếu Action = 1, thực hiện cập nhật dữ liệu
	begin
	update Category set	[Name] = @Name, [Type] = @Type where ID = @ID
	end
else if @Action = 2 -- Nếu Action = 2, thực hiện xóa dữ liệu
	begin
